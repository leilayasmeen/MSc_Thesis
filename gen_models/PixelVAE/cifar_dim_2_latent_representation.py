...
import random
import tflib as lib
import tflib.sampling_loop_cifar_filter_3_dim_2
...
OUT_DIR = DATASET + '_representation_filter_3_dim_2'
...
if SETTINGS == 'mnist_256':
    ...
    LATENT_DIM_2 = 2 #LEILA EDIT: was previously 512
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    if MODE == 'one_level':
        ...
        def generate_and_save_samples(tag):
            ...
            train_latents = np.zeros((1,LATENT_DIM_2)).astype('float32')
            dev_latents = np.zeros((1,LATENT_DIM_2)).astype('float32')
            test_latents = np.zeros((1,LATENT_DIM_2)).astype('float32')
        
            # Reshape image files
            x_train_set = x_train_set.reshape(-1, N_CHANNELS, HEIGHT, WIDTH)
            y_train_set = y_train_set.reshape(-1, 1)
            x_dev_set = x_dev_set.reshape(-1, N_CHANNELS, HEIGHT, WIDTH)
            y_dev_set = y_dev_set.reshape(-1, 1)
            x_test_set = x_test_set.reshape(-1, N_CHANNELS, HEIGHT, WIDTH)
            ...
            for j in range(x_train_set.shape[0]):
               ...
               train_latents = np.concatenate((train_latents, latestlatents), axis=0)
            train_latents = np.delete(train_latents, (0), axis=0)
            x_train_set_array = np.array(x_train_set)
            y_train_set_array = np.array(y_train_set)
            train_latents_array = np.array(train_latents)
            x_train_set_array = x_train_set_array.astype(np.uint8)
            np.save(OUT_DIR + '/' + 'x_train_set_array', x_train_array)
            np.save(OUT_DIR + '/' + 'y_train_set_array', y_train_array) 
            np.save(OUT_DIR + '/' + 'train_latents_array', train_latents_array)
            
            # Dev set
            for j in range(x_dev_set.shape[0]):
               latestlatents = enc_fn(x_dev_set[j,:].reshape(1, N_CHANNELS, HEIGHT, WIDTH))
               latestlatents = latestlatents.reshape(-1,LATENT_DIM_2)
               dev_latents = np.concatenate((dev_latents, latestlatents), axis=0)
            dev_latents = np.delete(dev_latents, (0), axis=0)
            x_dev_set_array = np.array(x_dev_set)
            y_dev_set_array = np.array(y_dev_set)
            dev_latents_array = np.array(dev_latents)
            x_dev_set_array = x_dev_set_array.astype(np.uint8)
            np.save(OUT_DIR + '/' + 'x_dev_set_array', x_dev_array)
            np.save(OUT_DIR + '/' + 'y_dev_set_array', y_dev_array) 
            np.save(OUT_DIR + '/' + 'dev_latents_array', dev_latents_array)
            
            # Test set
            for j in range(x_test_set.shape[0]):
               latestlatents = enc_fn(x_test_set[j,:].reshape(1, N_CHANNELS, HEIGHT, WIDTH))
               latestlatents = latestlatents.reshape(-1,LATENT_DIM_2)
               test_latents = np.concatenate((test_latents, latestlatents), axis=0)
            test_latents = np.delete(test_latents, (0), axis=0)
            x_test_set_array = np.array(x_test_set)
            y_test_set_array = np.array(y_test_set)
            test_latents_array = np.array(test_latents)
            x_test_set_array = x_test_set_array.astype(np.uint8)
            np.save(OUT_DIR + '/' + 'x_test_set_array', x_test_array)
            np.save(OUT_DIR + '/' + 'y_test_set_array', y_test_array) 
            np.save(OUT_DIR + '/' + 'test_latents_array', test_latents_array)
    ...
    lib.sampling_loop_cifar_filter_3_dim_2.sampling_loop( 
        session=session,
